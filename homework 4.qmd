...
Null = the predictor variable does not predict the reponse variable 
...
Null Hypothesis:
Mathematical: 
Biological: weight is in grams, length is in milimites 
...
gg_miss_var(fish_df) 
```
When running the function gg_miss_var, a total of 

we have found that there are 199 missing values for weight. 

489 - 290 
With the weight column demonstrating it has 199 missing values and the length column demonstrating there are no missing values, we can assign the weight to be a dependent variable and the length to be the independent variable. 
...
ggplot(data = troutperch_Na, aes(x = length, y = weight)) + 
  ...
  labs(title = "Troutperch lengths and weight")+
...
The diagnostic plot "Residual vs Fitted" visually implies that the data is heteroscedasticity.
...
plot_predictions <- ggplot(data = troutperch_Na, 
 ...
 labs(x = "Length (mm)",
      y = "weight (g)",
      title = "Prediction of troutperch"
      )
